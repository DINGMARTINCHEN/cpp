...
const COLORREF colors[MAX_POINTS] = {
    ...
    RGB(255, 192, 203), // Ç³·ÛÉ« (Light Pink)

    RGB(255, 182, 193), // µ­·Ûºì (LightPink)

    RGB(255, 105, 180), // ÈÈ·Ûºì (HotPink)

    RGB(255, 20, 147),  // Éî·ÛÉ« (DeepPink)

    RGB(219, 112, 147), // Å¨·Ûºì (PaleVioletRed)

    RGB(255, 174, 185), // Ç³Ãµ¹åºì (LightPink)

    RGB(255, 0, 144)    // ×ÏºìÉ« (Crimson)
...
void create_data() {
    ...
    for (int frame = 0; frame < NUM_FRAMES; ++frame) {
        ...
        settextstyle(100, 0, _T("ËÎÌå"));

        settextcolor(RGB(219, 112, 147));

        outtextxy(-150, -50, _T("ÎÒ°®Äã"));



        saveimage(_T("°®ÐÄ.png"), &images[frame]);

        setorigin(0, 0);

        setaspectratio(1, 1);

        loadimage(&images[frame], _T("°®ÐÄ.png"));
