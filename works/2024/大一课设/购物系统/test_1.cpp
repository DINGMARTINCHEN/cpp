...
void product_find(int pnum)
{
    ...
    // "product_food_data.txt"  1
    // "product_sport_data.txt"  2
    // "product_cloth_data.txt"  3
    // "product_pens_data.txt"  4
    // "product_machine_data.txt"  5
    string name;
    switch(pnum)
    {
        case 1: name="product_food_data.txt";
        case 2: name="product_sport_data.txt";
        case 3: name="product_cloth_data.txt";
        case 4: name="product_pens_data.txt";
        case 5: name="product_machine_data.txt";
    }
...
void product_up_to_date(int pnum)
{
    ...
    ofstream aaoutfile(name, ios::out);//清空原来仓库文件
    ofstream outfile(name, ios::app);//追加写入文件
    for(int i=0;i<product_size[pnum];i++)
    {
        string name;
        int num,stock,price;
...
{
    ofstream aaaoutfile("history_data.txt", ios::out);//清空文件
...
void shop_car()
{
    ...
    cout<<endl<<"如：1 2 3 4 5 6 @  代表1类的2号买3件；和4类的5号买6件";
    string aa,bb,cc;
    while(1)
    {
        cin>>aa>>bb>>cc;
...
void admin_space()
{
    ...
    cin>>choice;
    cout<<"请选择功能："<<endl;
    cout<<"1、添加货品信息"<<endl<<"2、查询货品信息"<<endl<<"输入任意键退出系统"<<endl<<endl;
    int choice_1;
    cin>>choice_1;
    if(choice_1==1)
    {
...
void up_to_date_product(int pnum)//把所有文件的数据读入数组，方便后续排序，更改文件等；同时更新上一次操作导致的仓库余量不同
{
    ...
    ifstream infile(name, ios::in);//读取文件
    for(int i=0;i<product_size[pnum];i++)
    {
        string a,b,c,d;
        infile>>a;
...
int main()
{
    for(int i=1;i<=5;i++)
    {
        up_to_date_product(i);//更新仓库货品数量
    }
    cout<<"请选择角色：";
    cout<<endl<<"1、管理员"<<endl<<"2、消费者"<<endl<<"输入任意键退出程序"<<endl<<endl;
    int choice_1;
    cin>>choice_1;
    if(choice_1==1)
    {
